...
{
    auto headerHash = hash256_from_bytes(_headerHash.data());
    auto& context = create_epoch_context_full(epoch);
    auto result = hash((const epoch_context_full&)context, headerHash, _nonce);
